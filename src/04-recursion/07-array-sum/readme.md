...
```mermaid
graph TD
  A["START | FINISH"] -->|"CALL: arraySum([1, 2, 3, 4, 5])"| B["arr[0] + arraySum([2, 3, 4, 5]) <br> 1 + 14 = 15"]
  B -->|"CALL: arraySum([2, 3, 4, 5])"| C["arr[0] + arraySum([3, 4, 5]) <br> 2 + 12 = 14"]
  C -->|"CALL: arraySum([3, 4, 5])"| D["arr[0] + arraySum([4, 5]) <br> 3 + 9 = 12"]
  D -->|"CALL: arraySum([4, 5])"| E["arr[0] + arraySum([5]) <br> 4 + 5 = 9"]
  E -->|"CALL: arraySum([5])"| F["arr[0] + arraySum([]) <br> 5 + 0 = 5"]
  F -->|"CALL: arraySum([])"| G[BASE CASE MET, UNWIND THE CALLSTACK]
  G -->|RETURN: 0| F
  F -->|RETURN: 5| E
  E -->|RETURN: 9| D
  D -->|RETURN: 12| C
